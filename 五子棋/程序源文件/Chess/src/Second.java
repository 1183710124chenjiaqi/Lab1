...
public class Second extends JFrame implements MouseListener {
	...
	public void mousePressed(MouseEvent e) {
		...
		if (canPlay) {
			if (x > 10 && x <= 660 && y >= 70 && y <= 720) {// 让鼠标在棋盘范围内
				if ((x - 20) % 35 > 17) {
					x = (x - 20) / 35 + 1;
				} else {
					x = (x - 20) / 35;
			...
			// 悔棋
			if (x >= 663 && x <= (663 + 110) && y >= 400 && y <= (400 + 50)) {
				if (Xposition.size() != 0)
					if (JOptionPane.showConfirmDialog(this, (isblack == true ? "白方悔棋,黑方是否同意？" : "黑方悔棋，白方是否同意？")) == 0) {
			...
			if (x >= 663 && x <= 773 && y >= 470 && y <= 520) {
				if (JOptionPane.showConfirmDialog(this, (isblack == true ? "黑方请求和棋,白方是否同意？" : "白方请求和棋，黑方是否同意？"), "提示",
			...
			if (x >= 663 && x <= 773 && y >= 540 && y <= 590) {
				if (JOptionPane.showConfirmDialog(this, "是否认输?", "提示", JOptionPane.OK_CANCEL_OPTION) == 0)
